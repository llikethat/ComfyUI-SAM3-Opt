...
def build_sam3_video_model(
    ...
    if checkpoint_path is not None:
        ...
        # MEMORY EFFICIENT: Convert all checkpoint tensors to float32 to avoid dtype mismatch
        # SAM3's internal processing creates float32 tensors, so model weights must also be
        # float32 for compatibility. This prevents "mat1 and mat2 must have same dtype" errors.
        converted_count = 0
        for key in ckpt:
            if isinstance(ckpt[key], torch.Tensor) and ckpt[key].is_floating_point():
                if ckpt[key].dtype != torch.float32:
                    ckpt[key] = ckpt[key].to(torch.float32)
                    converted_count += 1
        if converted_count > 0:
            print(f"[SAM3] Converted {converted_count} tensors to float32 for compatibility")
