...
class LoadSAM3Model:
    ...
    def load_model(self, model_path):
        ...
        # MEMORY EFFICIENT: Ensure model is in float32 to avoid dtype mismatch errors
        # SAM3's internal processing creates float32 tensors
        model = video_predictor.model
        try:
            param_dtype = next(model.parameters()).dtype
            if param_dtype != torch.float32:
                print(f"[SAM3] Converting model from {param_dtype} to float32 for compatibility...")
